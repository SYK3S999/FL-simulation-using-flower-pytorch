...
def generate_client_fn(trainloaders, valloaders, num_classes):
    ...
    def client_fn(context: Context):
        # Extract the client ID from the context
        cid = int(context.properties["cid"])  # Assuming client IDs are integers
        
        # Return a FlowerClient using the appropriate dataloaders for the client ID
        return FlowerClient(
            trainloader=trainloaders[cid],
            vallodaer=valloaders[cid],
            num_classes=num_classes,
        )
    
    # Return the updated client function
    return client_fn
